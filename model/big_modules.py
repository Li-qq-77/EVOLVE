...
class KeyProjection(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        ...
        self.pix_feat_proj2 = nn.Conv2d(in_dim, mid_dim, kernel_size=1)
        self.offset_evt = nn.Conv2d(mid_dim, 3 * 3 * 2, kernel_size=3, padding=1)

        self.regular_conv = nn.Conv2d(in_channels=mid_dim,
    ...
    def forward(self, x: torch.Tensor, evt: torch.Tensor, *, need_s: bool,
                need_e: bool) -> (torch.Tensor, torch.Tensor, torch.Tensor):
        h, w = x.shape[-2:]
        x = self.pix_feat_proj(x)
        e = self.pix_feat_proj2(evt)

        max_offset = max(h, w)/4.
        offset = self.offset_evt(e).clamp(-max_offset, max_offset)
        ...
        return self.key_proj(x), shrinkage, selection, offset

class EventKeyProjection(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        super().__init__()
        in_dim = model_cfg.pixel_encoder.ms_dims[0]
        #
    ...
    def forward(self, x: torch.Tensor, *, need_s: bool,
                need_e: bool) -> (torch.Tensor, torch.Tensor, torch.Tensor):
        x = self.pix_feat_proj(x)
        # e = self.pix_feat_proj2(e)
        # e = self.key_proj2(e)

        # x = self.transformer_1(x, e)
